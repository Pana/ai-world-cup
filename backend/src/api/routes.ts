  ...
  app.get("/api/v1/models/:slug", async (request, reply) => {
    ...
    const history = await queryRows<RowDataPacket[]>(
      `SELECT
         ...
         m.home_score_after_extra_time AS actualHomeScoreAfterExtraTime,
         m.away_score_after_extra_time AS actualAwayScoreAfterExtraTime,
         m.home_penalty_score AS actualHomePenaltyScore,
         m.away_penalty_score AS actualAwayPenaltyScore,
         m.result_type AS resultType,
         COALESCE(SUM(ps.points), 0) AS points,
         pv.version_key AS promptVersion,
